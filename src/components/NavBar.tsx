import React, { useEffect, useState } from 'react'
import works from '../data/works'
import { StaticImage } from 'gatsby-plugin-image'
import { useScrollPosition } from '@n8tb1t/use-scroll-position'
import { scrollTo, toggleWork } from '../utils'
import { Popover } from 'react-tiny-popover'

function WorksNav() {
  const [open, setOpen] = useState(false)
  return (
    <Popover
      isOpen={open}
      positions="bottom"
      align="start"
      padding={4}
      content={
        <ul className="menu rounded-box shadow-xl p-2 bg-base-100 w-40">
          {works.map(work => (
            <li key={work.name}>
              <a onClick={
                () => {
                  toggleWork(work.id)
                  setOpen(false)
                }
              }>
                {work.name}
              </a>
            </li>
          ))}
        </ul>
      }
      onClickOutside={() => setOpen(false)}
    >
      <a className={`submenu-a ${open ? 'open' : ''}`} onClick={() => setOpen(v => !v)}>项目</a>
    </Popover>
  )
}

function ThemeToggler() {
  // globalThis for ssr
  const [checked, setChecked] = useState(globalThis.localStorage?.getItem('night') === 'true')
  useEffect(() => {
    document.documentElement.dataset.theme = checked ? 'night' : 'winter'
    localStorage.setItem('night', String(checked))
  }, [checked])
  return (
    <label className="swap swap-rotate ml-4 mr-2">
      <input type="checkbox" checked={checked} onChange={evt => setChecked(evt.target.checked)} />
      <svg className="swap-off fill-current w-6 h-6" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M5.64,17l-.71.71a1,1,0,0,0,0,1.41,1,1,0,0,0,1.41,0l.71-.71A1,1,0,0,0,5.64,17ZM5,12a1,1,0,0,0-1-1H3a1,1,0,0,0,0,2H4A1,1,0,0,0,5,12Zm7-7a1,1,0,0,0,1-1V3a1,1,0,0,0-2,0V4A1,1,0,0,0,12,5ZM5.64,7.05a1,1,0,0,0,.7.29,1,1,0,0,0,.71-.29,1,1,0,0,0,0-1.41l-.71-.71A1,1,0,0,0,4.93,6.34Zm12,.29a1,1,0,0,0,.7-.29l.71-.71a1,1,0,1,0-1.41-1.41L17,5.64a1,1,0,0,0,0,1.41A1,1,0,0,0,17.66,7.34ZM21,11H20a1,1,0,0,0,0,2h1a1,1,0,0,0,0-2Zm-9,8a1,1,0,0,0-1,1v1a1,1,0,0,0,2,0V20A1,1,0,0,0,12,19ZM18.36,17A1,1,0,0,0,17,18.36l.71.71a1,1,0,0,0,1.41,0,1,1,0,0,0,0-1.41ZM12,6.5A5.5,5.5,0,1,0,17.5,12,5.51,5.51,0,0,0,12,6.5Zm0,9A3.5,3.5,0,1,1,15.5,12,3.5,3.5,0,0,1,12,15.5Z"/></svg>
      <svg className="swap-on fill-current w-6 h-6" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M21.64,13a1,1,0,0,0-1.05-.14,8.05,8.05,0,0,1-3.37.73A8.15,8.15,0,0,1,9.08,5.49a8.59,8.59,0,0,1,.25-2A1,1,0,0,0,8,2.36,10.14,10.14,0,1,0,22,14.05,1,1,0,0,0,21.64,13Zm-9.5,6.69A8.14,8.14,0,0,1,7.08,5.22v.27A10.15,10.15,0,0,0,17.22,15.63a9.79,9.79,0,0,0,2.1-.22A8.11,8.11,0,0,1,12.14,19.73Z"/></svg>
    </label>
  )
}

function NavBar() {

  const [show, setShow] = useState(false)
  useScrollPosition(({ currPos }) => {
    setShow(currPos.y >= window.innerHeight)
  }, [setShow], undefined, true, 300)

  return (
    <div className={`navbar ${show ? 'bg-base-100' : 'bg-opacity'} fixed top-0 left-0 right-0 z-10`}>
      <div className={`flex-1 flex gap-4 pl-6 ${show ? 'visible' : 'invisible'}`}>
        <div className="avatar">
          <div className="w-10 rounded-full">
            <StaticImage src="../images/logo.png" alt="logo" layout="fixed" width={40} height={40} />
          </div>
        </div>
        <a className="hidden xs:inline text-xl font-bold">Flora Studio</a>
      </div>
      <div className="flex-none flex items-center">
        <ul className="menu menu-horizontal px-1">
          <li><WorksNav /></li>
          <li><a onClick={() => scrollTo('members')}>成员</a></li>
          <li><a onClick={() => scrollTo('service')}>联系</a></li>
        </ul>
        <ThemeToggler />
      </div>
    </div>
  )
}

export default NavBar
